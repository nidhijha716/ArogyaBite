...
# # Load the Word2Vec model and datasets
# try:
#     with open('models/word2vec_model.pkl', 'rb') as f:
#         word2vec_model = pickle.load(f)
# except Exception as e:
...
# try:
#     # Dataset for recipe recommendation
#     df_recipes = pd.read_csv('datasets/Food_Recipes_Cleaned.csv')
#     # Dataset for nutrition analysis and diet planning
#     df_nutrition = pd.read_csv('datasets/dataset.csv')

#     # Print column names to debug
#     print("Recipe dataset columns:", df_recipes.columns.tolist())
#     print("Nutrition dataset columns:", df_nutrition.columns.tolist())
# except Exception as e:
#     print(f"Error loading datasets: {e}")

# # Preprocessing and recommendation helpers
# def preprocess_ingredients(ingredient_list):
#     return [ingredient.strip().lower() for ingredient in ingredient_list]
...
#     recipe_vectors = []
#     for ingredients in df_recipes['Ingredients_name']:
#         if not isinstance(ingredients, str):
#             recipe_vectors.append(np.zeros(word2vec_model.vector_size))
#             continue
#         ingredient_list = [i.strip().lower() for i in ingredients.split(',')]
...
#     similarity_scores = cosine_similarity([preference_vector], recipe_vectors)[0]
#     df_recipes['similarity_score'] = similarity_scores
...
#     filtered_df = df_recipes[~df_recipes['Ingredients_name'].apply(contains_allergen)]
#     top_recommendations = filtered_df.sort_values(by='similarity_score', ascending=False).head(5)
#     return top_recommendations[['Name', 'Ingredients_name', 'similarity_score']].to_dict(orient='records')
...
#     }
#     protein_pct, fat_pct, carbs_pct = macros.get(goal, macros['maintain'])
...
#     recommended_recipes = recommend_recipes(allergies, preferences)
#     meal_types = ["Breakfast", "Lunch", "Dinner"] + [f"Snack {i+1}" for i in range(meals_per_day - 3)]

#     meal_distribution = [0.3, 0.4, 0.3]
#     if meals_per_day == 4:
#         meal_distribution = [0.25, 0.35, 0.3, 0.1]
#     elif meals_per_day == 5:
#         meal_distribution = [0.25, 0.3, 0.25, 0.1, 0.1]

#     while len(recommended_recipes) < 7 * meals_per_day:
...
#             cal = daily_calories * meal_distribution[i]
#             meal_plan[day][meal_type] = {
#                 "recipe": recommended_recipes[index]['Name'],
#                 "ingredients": recommended_recipes[index]['Ingredients_name'],
#                 "calories": round(cal),
#                 "protein": round(protein_grams * (cal / daily_calories)),
#                 "carbs": round(carbs_grams * (cal / daily_calories)),
#                 "fat": round(fat_grams * (cal / daily_calories))
#             }
#             index += 1
...
# def recommend():
#     data = request.json
#     return jsonify(recommend_recipes(data.get('allergies', []), data.get('preferences', [])))

# @app.route('/diet-plan', methods=['POST'])
# def diet_plan():
#     data = request.json
#     return jsonify(generate_meal_plan(
#         data.get('height', 170),
#         data.get('weight', 70),
#         data.get('age', 30),
...
# @app.route('/food-alternatives', methods=['POST'])
...
#     result = find_food_alternatives(food_name, allergies, preferences, num_recommendations)
